...
from app.modules.proxy.load_balancer import NO_ALTERNATE_ACCOUNTS, effective_account_concurrency_caps
...
class _HTTPBridgeRequestSubmitMixin:
    ...
    async def _retry_http_bridge_precreated_request(self: Any, session: "_HTTPBridgeSession") -> bool:
        ...
        try:
            ...
            # A request-local replay can have no alternate account even while the
            # pool is healthy. Keep the initial upstream close/idle failure as the
            # authoritative error rather than overwriting it with that local fact.
            if request_state.error_code_override == NO_ALTERNATE_ACCOUNTS:
